...
final.data <- final.data %>%
  mutate(star_rounded = case_when(
    Star_Rating >= 3.0 & Star_Rating < 3.5 ~ 3,
    Star_Rating >= 3.5 & Star_Rating < 4.0 ~ 3.5,
    Star_Rating >= 4.0 & Star_Rating < 4.5 ~ 4,
    Star_Rating >= 4.5 & Star_Rating < 5.0 ~ 4.5,
    Star_Rating >= 5.0 ~ 5,
    TRUE ~ NA_real_))

star_rating_counts <- final.data %>%
  filter(!is.na(star_rounded)) %>% 
  group_by(star_rounded) %>%
  summarize(num_plans = n())
